...
void DualMC33926MotorShield::setM1Speed(int speed)
{
  ...
  #ifdef DUALMC33926MOTORSHIELD_TIMERS_AVAILABLE
    if (_M1PWM == _M1PWM_TIMER1_PIN && _M2PWM == _M2PWM_TIMER1_PIN)
    {
      OCR1B = speed;
    }
    else if (_M1PWM == _M1PWM_TIMER5_PIN && _M2PWM == _M2PWM_TIMER5_PIN)
    {
        if (speed > 400)  // Max PWM dutycycle
            speed = 400;
        OCR5C = speed;
    }else if (_M1PWM == _M1PWM_TIMER3_PIN && _M2PWM == _M2PWM_TIMER3_PIN)
    {
        OCR3B = speed;
    } else
    {
